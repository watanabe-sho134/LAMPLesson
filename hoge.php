...
try {
    $pdo = new PDO('mysql:host=localhost;dbname=iaia;charset=utf8','root'
...
if ($item_name && isset($_POST["btn"])){
    ...
    $stmt = $pdo -> prepare("INSERT INTO ueue(name, point) VALUES (:name, :point)");
    $stmt->bindParam(':name', $item_name, PDO::PARAM_STR);
    $stmt->bindValue(':point', $item_point, PDO::PARAM_INT);
    $stmt->execute();
}
$select_sql = "SELECT * FROM ueue";
